...
function getPageHtml(url,total,max,totalPage,currentPage,elementId,targetFunction){
	...
	'<div class="page">';
	if(currentPage>1){
		html+='<a onclick="'+targetFunction+'(\''+url+'&max='+max+'&page=1\')" href="javaScript:void(0)" class="page_a">首页</a>'+
			'<a onclick="'+targetFunction+'(\''+url+'&max='+max+'&page='+(currentPage-1)+'\')" href="javaScript:void(0)" class="page_a">上页</a>';
	}
	...
	if(totalPage<=10){
		for(var i=1;i<=totalPage;i++){
			if(i==currentPage)
				...
				html+='<a  onclick="'+targetFunction+'(\''+url+'&max='+max+'&page='+i+'\')" href="javaScript:void(0)" class="page_a">'+i+'</a>';
		}
	}else{
		if(currentPage<=5){
			for(var i=1;i<11;i++){
	...
	if(currentPage<totalPage){
		html+='<a onclick="'+targetFunction+'(\''+url+'&max='+max+'&page='+(currentPage+1)+'\')" href="javaScript:void(0)" class="page_a">下页</a>'+
			'<a onclick="'+targetFunction+'(\''+url+'&max='+max+'&page='+totalPage+'\')" href="javaScript:void(0)" class="page_a">尾页</a>';
	}
	
	html+='<input type="text" id="pageCodeInput" />/'+totalPage+
		'<a class="page_a" onclick="'+targetFunction+'(\''+url+'&max='+max+'\',1,0)" href="javaScript:void(0)">跳转</a>'+
		'</div>'+
		'<div class="clear"></div>'+
		'</div>';
